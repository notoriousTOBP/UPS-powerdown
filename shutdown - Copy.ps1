...
$xenkill = {
	...
	try{connect-xenserver -url $master -creds $creds}catch{"$(Get-Date -f "HH:mm:ss") - $master -  Connecting to Xenhost $args FAILED - error returned: $($_)" >> $log; exit}
	...
	$script = {
		...
		try{get-xenvm $vm.name_label | invoke-xenvm -xenaction cleanshutdown}catch{"$(Get-Date -f "HH:mm:ss") - $pool - Shutting down $($vm.name_label) FAILED - error returned: $($_)" >> $log; exit}
		}
	...
	if(!$pool){$pool = @(get-xenhost).name_label}
	#Shutdown all running VMs but not control domain VMs:
	"$(Get-Date -f "HH:mm:ss") - $pool - Launching VM shutdown jobs" >> $log
	foreach($vm in get-xenvm | where{$_.power_state -eq "Running" -and $_.is_control_domain -eq $false}) {"$(Get-Date -f "HH:mm:ss") - $pool - Shutting down $($vm.name_label)" >> $log;start-job $script -args $master,$creds,$vm,$log,$pool} 
	...
	try{get-xenhost | where{$_.opaque_ref -ne @(Get-XenPool).master.opaque_ref} | invoke-xenhost -xenaction disable}catch{"$(Get-Date -f "HH:mm:ss") - Disabling slave host $($_.name_label) FAILED - error returned: $($_)" >> $log}
	"$(Get-Date -f "HH:mm:ss") - $pool - Slave hosts disabled. Shutting down slave hosts" >> $log
	#Shutdown all hosts in pool but not master:
	try{get-xenhost | where{$_.opaque_ref -ne @(Get-XenPool).master.opaque_ref} | invoke-xenhost -xenaction shutdown}catch{"$(Get-Date -f "HH:mm:ss") - Shutting down slave host $($_.name_label) FAILED - error returned: $($_)" >> $log}
	"$(Get-Date -f "HH:mm:ss") - $pool - Slave hosts offline. Disabling pool master" >> $log
	#Disable pool master:
	try{get-xenhost @(Get-XenPool).master | invoke-xenhost -xenaction disable}catch{"$(Get-Date -f "HH:mm:ss") - Disabling pool master $args FAILED - error returned: $($_)" >> $log}
	"$(Get-Date -f "HH:mm:ss") - $pool - Pool master disabled. Shutting down pool master" >> $log
	#Shutdown pool master:
	try{get-xenhost @(Get-XenPool).master | invoke-xenhost -xenaction shutdown}catch{"$(Get-Date -f "HH:mm:ss") - Shutting down pool master $args FAILED - error returned: $($_)" >> $log}
	#Disconnect
